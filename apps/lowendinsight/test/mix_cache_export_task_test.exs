...
defmodule Mix.Tasks.CacheExportTaskTest do
  ...
  describe "run/1 error paths" do
    ...
    test "reports error for nonexistent repo file" do
      assert catch_exit(Mix.Tasks.Lei.Cache.Export.run(["/nonexistent/file/repos.txt"])) ==
               {:shutdown, 1}
    ...
    test "reports error when JSONL produces empty reports" do
      tmp_file =
        Path.join(System.tmp_dir!(), "lei-empty-jsonl-#{:rand.uniform(100_000)}.jsonl.gz")

      # Write empty gzipped content (empty JSONL = no reports)
      File.write!(tmp_file, :zlib.gzip(""))
      on_exit(fn -> File.rm(tmp_file) end)

      assert catch_exit(Mix.Tasks.Lei.Cache.Export.run(["--input", tmp_file])) == {:shutdown, 1}
    ...
    test "reports push error for unreachable registry" do
      ...
      tmp_file =
        Path.join(System.tmp_dir!(), "lei-pushfail-jsonl-#{:rand.uniform(100_000)}.jsonl.gz")
    ...
    test "exports from valid JSONL input" do
      ...
      tmp_file =
        Path.join(System.tmp_dir!(), "lei-valid-jsonl-#{:rand.uniform(100_000)}.jsonl.gz")
    ...
    test "exports from repo file with local file:// URLs" do
      ...
      tmp_file =
        Path.join(System.tmp_dir!(), "lei-repos-local-#{:erlang.unique_integer([:positive])}.txt")

      File.write!(tmp_file, "# This is a comment\nfile:///#{cwd}\n\n")
      on_exit(fn -> File.rm(tmp_file) end)

      output_dir =
        Path.join(System.tmp_dir!(), "lei-export-local-#{:erlang.unique_integer([:positive])}")
    ...
    test "push error with host-only registry (no repo path)" do
      ...
      tmp_file =
        Path.join(
          System.tmp_dir!(),
          "lei-hostonly-jsonl-#{:erlang.unique_integer([:positive])}.jsonl.gz"
        )
      ...
      output_dir =
        Path.join(System.tmp_dir!(), "lei-hostonly-out-#{:erlang.unique_integer([:positive])}")
    ...
    test "exports with skipping push (default)" do
      ...
      tmp_file =
        Path.join(
          System.tmp_dir!(),
          "lei-skip-jsonl-#{:erlang.unique_integer([:positive])}.jsonl.gz"
        )
      ...
      output_dir =
        Path.join(System.tmp_dir!(), "lei-skip-out-#{:erlang.unique_integer([:positive])}")

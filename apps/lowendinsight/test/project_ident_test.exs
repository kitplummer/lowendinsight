...
defmodule ProjectIdentTest do
  ...
  describe "find_files/2 (unit tests)" do
    test "finds mix files in current directory" do
      ...
      mix_type = %ProjectType{name: :mix, path: "", files: ["mix.exs"]}
  ...
  test "is_maven?(repo)", %{tmp_path: tmp_path, project_types: project_types} do
    {:ok, repo} =
      GitModule.clone_repo("https://github.com/kitplummer/snyk-maven-plugin", tmp_path)

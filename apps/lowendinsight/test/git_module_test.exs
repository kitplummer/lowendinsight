...
defmodule GitModuleTest do
  ...
  # Most tests require network access (setup_all clones multiple repos)
  @moduletag :network
  @moduletag :long
  ...
  test "get commit count for default branch", %{repo: repo} do
    assert {:ok, 7} = GitModule.get_total_commit_count(repo)
  end

  test "get commit count for default branch for path", %{this_repo: repo} do
    assert {:ok, _count} = GitModule.get_total_commit_count(repo)
  end
  ...
  test "get contribution maps", %{kitrepo: kitrepo} do
    ...
    expected_array = [
      %{
        contributions: 358,
        name: "Ben Morris",
        last_contribution_date: "2018-12-27T10:24:58-08:00"
      },

defmodule Lei.AgenticDetectorTest do
  ...
  describe "classify_ratio/1" do
    ...
    test "classifies 0.29 as human (just below mixed threshold)" do
      assert AgenticDetector.classify_ratio(0.29) == {:ok, "human"}
    end

    test "classifies 0.3 as mixed (at lower threshold boundary)" do
    ...
    test "classifies 0.7 as mixed (at upper threshold boundary, not agent)" do
      assert AgenticDetector.classify_ratio(0.7) == {:ok, "mixed"}
    end

    test "classifies 0.71 as agent (just above upper threshold)" do
    ...
    test "classifies ratio just below lower threshold as human" do
      assert AgenticDetector.classify_ratio(0.2999) == {:ok, "human"}
    end

    test "classifies ratio just above upper threshold as agent" do
      assert AgenticDetector.classify_ratio(0.7001) == {:ok, "agent"}
    end
  end

...
defmodule Mix.Tasks.ZarfGateTaskTest do
  ...
  describe "run/1 error paths" do
    test "reports error for nonexistent path" do
      assert catch_exit(Mix.Tasks.Lei.ZarfGate.run(["--path", "/nonexistent/path/12345"])) ==
               {:shutdown, 1}
  ...
  describe "run/1 with minimal project (no network)" do
    setup do
      tmp_dir =
        Path.join(System.tmp_dir!(), "lei_zarf_test_#{:erlang.unique_integer([:positive])}")
    ...
    test "writes output to file for minimal project", %{tmp_dir: tmp_dir} do
      output =
        Path.join(
          System.tmp_dir!(),
          "lei-zarf-output-#{:erlang.unique_integer([:positive])}.json"
        )
    ...
    test "writes output to file and shows written message in non-quiet mode", %{tmp_dir: tmp_dir} do
      output =
        Path.join(
          System.tmp_dir!(),
          "lei-zarf-nonquiet-#{:erlang.unique_integer([:positive])}.json"
        )

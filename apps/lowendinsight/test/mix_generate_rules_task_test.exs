...
defmodule Mix.Tasks.GenerateRulesTaskTest do
  ...
  setup do
    ...
    tmp_dir =
      Path.join(System.tmp_dir!(), "lei-rules-test-#{:erlang.unique_integer([:positive])}")
  ...
  test "generates copilot rules file", %{tmp_dir: tmp_dir} do
    ...
    assert File.exists?(
             Path.join(tmp_dir, ".github/instructions/lei-dependency-rules.instructions.md")
           )
  end
  ...
  test "build_thresholds uses opts when provided" do
    thresholds =
      Mix.Tasks.Lei.GenerateRules.build_thresholds(

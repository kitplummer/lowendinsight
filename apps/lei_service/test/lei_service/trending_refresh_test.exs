defmodule LeiService.TrendingRefreshTest do
  @moduledoc """
  ...
  alias LeiService.{Datastore, GithubTrending}
  ...
  describe "the size limit" do
    test "defaults to 250 MB and is configurable" do
      ...
      previous = Application.get_env(:lei_service, :trending_max_repo_size_kb)
      Application.put_env(:lei_service, :trending_max_repo_size_kb, 1_000)

      on_exit(fn ->
        if previous,
          do: Application.put_env(:lei_service, :trending_max_repo_size_kb, previous),
          else: Application.delete_env(:lei_service, :trending_max_repo_size_kb)
      end)
  ...
  describe "metrics/1" do
    test "reports each language's completion and the age of its report" do
      previous = Application.get_env(:lei_service, :languages)
      Application.put_env(:lei_service, :languages, @languages)
      on_exit(fn -> Application.put_env(:lei_service, :languages, previous) end)

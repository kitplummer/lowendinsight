defmodule LeiService.RemoteUrlTest do
  @moduledoc """
  ...
  describe "through the service" do
    ...
    test "Try It refuses a local path before any analysis" do
      ...
      LeiService.Datastore.delete_from_cache("file:///etc")

      conn = conn(:get, "/url=" <> URI.encode_www_form("file:///etc")) |> call()

      assert conn.status == 400
      refute LeiService.Datastore.in_cache?("file:///etc")
    end
    ...
    test "the analysis entry point refuses what the routes would" do
      assert {:error, _} =
               LeiService.Analysis.analyze("file:///etc", "lei-get", %{types: false})
    end
  end

  defp call(conn), do: LeiService.Endpoint.call(conn, @opts)
end

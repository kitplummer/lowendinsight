...
defmodule LeiService.EndpointTest do
  ...
  test "with no token, analysis is refused with a 402, never served" do
    ...
    Redix.command(:redix, [
      "DEL",
      LeiService.Datastore.cache_key("https://github.com/gbtestee/gbtestee")
    ])
  ...
  test "it returns 401 with no token on a route that is not paid" do
    conn = conn(:get, "/v1/usage") |> LeiService.Endpoint.call(@opts)
  ...
  test "it returns 200 with a valid payload when cached" do
    Redix.command(:redix, [
      "DEL",
      LeiService.Datastore.cache_key("https://github.com/kitplummer/git-author")
    ])
  ...
  test "POST with cache_mode async returns immediately with uuid and incomplete state" do
    ...
    Redix.command(:redix, ["DEL", LeiService.Datastore.cache_key(unique_url)])
  ...
  test "POST with cache_mode stale returns stale data when cached" do
    ...
    cache_key = LeiService.Datastore.cache_key(url)

defmodule LeiService.AnalysisTest do
  ...
  describe "process_urls/3" do
    test "returns error for invalid URLs" do
      ...
      assert {:error, "invalid URLs list"} ==
               LeiService.Analysis.process_urls(
                 ["htps://invalid-url"],
                 uuid,
                 start_time
               )
    ...
    test "returns complete immediately when all URLs are cached" do
      ...
      {:ok, json} = LeiService.Analysis.process_urls([url], uuid, start_time)
    ...
    test "queues analysis for uncached URLs and returns incomplete report" do
      ...
      # Ensure not cached
      key = LeiService.Datastore.cache_key(url)
  ...
  describe "analyze/3 cache interaction" do
    ...
    test "returns :miss on first call then :hit on second call" do
      ...
      {:ok, _report, status1} = LeiService.Analysis.analyze(url, "test", %{types: false})
      assert status1 == :miss

      {:ok, _report, status2} = LeiService.Analysis.analyze(url, "test", %{types: false})
      assert status2 == :hit
    end
  end
  ...
  describe "refresh_job/1" do
    test "completes job when all repos are now cached" do
      ...
      result = LeiService.Analysis.refresh_job(job)

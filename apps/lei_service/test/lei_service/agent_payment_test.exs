defmodule LeiService.AgentPaymentTest do
  @moduledoc """
  An agent that has never been here pays with stablecoin and is served (#147).

  Driven through LeiService.Endpoint, the thing deployed. #131 and #146
  ...
  describe "an agent that has never been here" do
    ...
    test "paying on /v1/analyze debits the paying org for the analysis it is served" do
      ...
      LeiService.Datastore.write_to_cache(url, %{
        data: %{repo: url},
    ...
    test "nothing records which repository a paying agent analysed (#149)" do
      ...
      :sys.get_state(LeiService.RequestLogger)
      org = Wallets.find_by_address(@payer)
      entries = Enum.filter(LeiService.RequestLogger.get_all(), &(&1.org_id == org.id))
      ...
      refute Enum.any?(LeiService.RequestLogger.get_all(), &(url in (&1.repo_urls || [])))
    end
  ...
  describe "what stays closed" do
    ...
    test "routes that are not paid still require authentication" do
      for {method, path} <- [
        ...
        conn = conn(method, path) |> LeiService.Endpoint.call(@opts)

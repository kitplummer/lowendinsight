defmodule LeiService.JobAccessTest do
  @moduledoc """
  ...
  setup do
    ...
    original = Application.get_env(:lei_service, :job_refresher)

    on_exit(fn ->
      if original,
        do: Application.put_env(:lei_service, :job_refresher, original),
        else: Application.delete_env(:lei_service, :job_refresher)
    end)
  ...
  test "polling an incomplete job starts work at most once per window", %{key: key} do
    ...
    Application.put_env(:lei_service, :job_refresher, fn job ->

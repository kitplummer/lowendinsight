...
defmodule LeiService.GithubTrendingTest do
  ...
  setup do
    on_exit(fn ->
      Task.Supervisor.children(LeiService.AnalysisSupervisor)
      |> Enum.map(fn child ->
        Task.Supervisor.terminate_child(LeiService.AnalysisSupervisor, child)
      end)
    end)
  end
  ...
  describe "OSS Insight API contract" do
    ...
    test "returns repos for elixir with expected structure" do
      {:ok, repos} = LeiService.GithubTrending.fetch_from_ossinsight("elixir")
    ...
    test "returns repos for multiple languages" do
      for lang <- ["python", "rust", "go"] do
        result = LeiService.GithubTrending.fetch_from_ossinsight(lang)
  ...
  describe "GitHub Search API contract" do
    ...
    @tag timeout: 60_000
    test "returns repos for elixir with expected structure" do
      {:ok, repos} = LeiService.GithubTrending.fetch_from_github_search("elixir")
      assert is_list(repos)
      assert length(repos) > 0
    ...
    test "returns repos for multiple languages" do
      for lang <- ["python", "rust", "go"] do
        result = LeiService.GithubTrending.fetch_from_github_search(lang)
  ...
  describe "fetch_trending_list/1 (layered fallback)" do
    ...
    test "returns repos regardless of which source responds" do
      {:ok, list} = LeiService.GithubTrending.fetch_trending_list("elixir")
      assert is_list(list)
  ...
  describe "capitalize_language/1" do
    test "maps lowercase language names to API display names" do
      assert LeiService.GithubTrending.capitalize_language("c++") == "C++"
      assert LeiService.GithubTrending.capitalize_language("c#") == "C#"
      assert LeiService.GithubTrending.capitalize_language("objective-c") == "Objective-C"
      assert LeiService.GithubTrending.capitalize_language("javascript") == "JavaScript"
      assert LeiService.GithubTrending.capitalize_language("typescript") == "TypeScript"
      assert LeiService.GithubTrending.capitalize_language("elixir") == "Elixir"
      assert LeiService.GithubTrending.capitalize_language("python") == "Python"
      assert LeiService.GithubTrending.capitalize_language("rust") == "Rust"
    end
  end
  ...
  describe "end-to-end refresh (network)" do
    ...
    test "refresh/1 publishes a complete report, and the page renders its rows" do
      ...
      assert {:ok, _uuid} = LeiService.GithubTrending.refresh(language)
      ...
      assert LeiService.GithubTrending.completed_at(language)

      conn = conn(:get, "/gh_trending/#{language}") |> LeiService.Endpoint.call(@opts)
  ...
  describe "get_current_gh_trending_report/1" do
    test "returns empty report when no UUID exists in Redis" do
      ...
      report = LeiService.GithubTrending.get_current_gh_trending_report("nonexistent")
  ...
  test "repositories at or over the size limit are not analysed" do
    alias LeiService.GithubTrending

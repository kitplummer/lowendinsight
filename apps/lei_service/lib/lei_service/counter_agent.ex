defmodule LeiService.CounterAgent do
  use Agent
  require Logger
  ...
  def new_counter(number_of_processes) do
    if !Process.whereis(:counter) do
      ...
      Agent.update(:counter, fn {proc, log} ->
        {proc, Map.put(log, :total, log.total + number_of_processes)}
      end)
    end
  ...
  def log_status({proc, log}) do
    ...
    cond do
      log.total > log.completed ->
        Logger.info(
          "completed #{round(completed / log.total * 100)}% " <>
            if(completed < log.total, do: " ", else: "") <>
            "|  running: #{map_size(proc) - completed}  |  total URLs: #{log.total}"
        )

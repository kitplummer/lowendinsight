...
defmodule LeiService.GithubTrending do
  ...
  def refresh_due(opts \\ []) do
    ...
    with_lock(lock_ms, fn token ->
      ...
      |> Keyword.get_lazy(:languages, fn ->
        Application.get_env(:lei_service, :languages)
      end)
  ...
  def refresh(language, opts \\ []) do
    ...
    analyze = Keyword.get(opts, :analyze, &LeiService.Analysis.process/3)
    uuid = UUID.uuid1()
  ...
  defp ensure_complete(uuid) do
    with {:ok, json} <- LeiService.Datastore.get_job(uuid),
  ...
  defp candidate_urls(list, repo_size) do
    ...
    num_of_repos = Application.get_env(:lei_service, :num_of_repos) || 5
  ...
  defp get_token() do
    if Application.fetch_env(:lei_service, :gh_token) == :error,
      do: "",
  ...
  def get_wait_time() do
    if Application.fetch_env(:lei_service, :wait_time) == :error,
      do: 1_800_000,
      else: Application.fetch_env!(:lei_service, :wait_time)
  end

  def check_repo_size?() do
    if Application.fetch_env(:lei_service, :check_repo_size?) == :error,
      do: false,
      else: Application.fetch_env!(:lei_service, :check_repo_size?)
  end

...
if [ "${INPUT_SARIF}" = "true" ]; then
    ...
    echo "Running LowEndInsight SARIF scan..."
    MIX_ENV=gha mix lei.sarif "${GITHUB_WORKSPACE}" --output "${GITHUB_WORKSPACE}/${INPUT_SARIF_FILE}"
    echo "SARIF report written to ${INPUT_SARIF_FILE}"
else

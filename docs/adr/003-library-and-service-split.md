...
- **`apps/lowendinsight` is the analyzer only**: git analysis, risk scoring,
  ...
  only `Lei.BatchCache`.
- **`apps/lowendinsight_get` is the service.** Every other `Lei.*` module lives
  ...
  `LowendinsightGet.Application` starts.
...
- The production database needs no migration: `schema_migrations` is shared,
  ...
  `LowendinsightGet.Release.migrate()` against a migrated database (nothing
...
- Follow-ups, in order:
  1. Move the service's settings from the `:lowendinsight` application env to
     `:lowendinsight_get`, so the library's env holds only analyzer settings.
  2. Prove the library stands alone: a CI job that builds a fresh project
     ...
     dependencies.
  3. Consider renaming `lowendinsight_get`, which is now the whole service.
